...
function UpdateOnboarding() {
    ...
    const isConfirmed = window.confirm('Apakah Anda yakin ingin memperbarui onboarding?');
    ...
    const formattedData = {
        ...
        pic_user_id: onboardingData.interviewer ? onboardingData.interviewer : onboarding.pic_user_id.user_id,
...
const deleteOnboarding = async (id) => {
    try {
        const response = await fetch(`http://127.0.0.1:8000/api/onboarding/delete-onboarding/${id}/`, {
            method: "DELETE",
        });
        const isConfirmed = window.confirm('Apakah Anda yakin ingin menghapus onboarding?');
                ...
                <React.Fragment key={onboarding.id}>
                    ...
                    <div className='update-interview'>
                        <div className="rectangle-style" style={rectangleStyle}>
                            <p style={{ marginTop: '20px', marginLeft: '23%', fontWeight: 'bold', fontSize: '32px', color: '#2A3E4B', position: 'absolute' }}>Ubah Jadwal Onboarding</p>
                            <form onSubmit={handleSubmit}>
                                ...
                                <input
                                    ...
                                    readOnly
                                />
                                <p style={{ marginTop: '280px', marginLeft: '7%', fontWeight: '600', fontSize: '14px', color: '#2A3E4B', position: 'absolute' }}>Tanggal Onboarding*</p>
                                <input
                            type="date"
                            style={{
                                borderRadius: '5px',
                            ...
                            value={onboardingData.datetime_start || (onboarding && onboarding.datetime_start ? onboarding.datetime_start.split('T')[0] : '')}
                            onChange={(e) => {
                                ...
                                const startTime = new Date(`${selectedDate}T${onboarding.datetime_start.slice(11, 16)}`);
                                ...
                                setOnboardingData(prevState => ({ ...prevState, datetime_start: selectedDate }));
                            }}
                            min={currentDateString}
                        />
                        <p style={{ marginTop: '380px', marginLeft: '7%', fontWeight: '600', fontSize: '14px', color: '#2A3E4B', position: 'absolute' }}>Waktu Mulai Onboarding*</p>
                        <input
                            type="time"
                            style={{
                                borderRadius: '5px',
                            ...
                            value={onboardingData.startTime || (onboarding && onboarding.datetime_start ? onboarding.datetime_start.slice(11, 16) : '')}
                            onChange={(e) => {
                                const selectedTime = e.target.value;
                                const formattedTime = selectedTime.slice(0, 5);
                                const startTime = new Date(`${onboardingData.datetime_start}T${formattedTime}`);
                                const currentTime = new Date();
                                const currentDateString = currentTime.toISOString().split('T')[0];
                                const currentTimeString = currentTime.toTimeString().slice(0, 5);

                                if (onboardingData.datetime_start === currentDateString && startTime < currentTime) {
                                    alert('Tanggal atau waktu mulai tidak valid. Pastikan tanggal dan waktu mulai sesuai.');
                                    return;
                                }
                        ...
                        />
                    <p style={{ marginTop: '480px', marginLeft: '7%', fontWeight: '600', fontSize: '14px', color: '#2A3E4B', position: 'absolute' }}>Waktu Berakhir Onboarding*</p>
                    <input
                    type="time"
                    style={{
                        borderRadius: '5px',
                    ...
                    value={onboardingData.endTime || (onboarding && onboarding.datetime_end ? onboarding.datetime_end.slice(11, 16) : '')}
                    onChange={(e) => {
                        const selectedTime = e.target.value;
                        const prevEndTime = onboarding && onboarding.datetime_end ? onboarding.datetime_end.slice(11, 16) : '';
                        
                        if (onboardingData.startTime && selectedTime < onboardingData.startTime) {
